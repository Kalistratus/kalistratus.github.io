...
var linkNav = document.querySelectorAll('[href^="#"]'), //выбираем все ссылки к якорю на странице
    V = 0.4;  // скорость прокрутки (чем меньше значение - тем больше скорость)
  ...
  var
    ...
    _actionClick = function (e) {
      var
        ...
        if (e.target.classList.contains(ClassName.INDICATOR_ACTIVE)){
            return;
        }
...
$(window).scroll(function() {
  if ($(this).scrollTop() > 1){
    ...
    $('div.header-logo').addClass("hidden");
    $('div.container-head').addClass("right");
  }
  else{
    $('header').removeClass("sticky");
    $('div.header-logo').removeClass("hidden");
    $('div.container-head').removeClass("right");
  }
});